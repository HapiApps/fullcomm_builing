...
typedef FilterCallback<T> = List<MyDropdownMenuEntry<T>> Function(List<MyDropdownMenuEntry<T>> entries, String filter);

typedef SearchCallback<T> = int? Function(List<MyDropdownMenuEntry<T>> entries, String query);
...
class _MyDropdownMenuState<T> extends State<MyDropdownMenu<T>> {
  ...
  TextEditingValue get _initialTextEditingValue {
    for (final MyDropdownMenuEntry<T> entry in filteredEntries) {
  ...
  void didUpdateWidget(MyDropdownMenu<T> oldWidget) {
    ...
    if (oldWidget.dropdownMenuEntries != widget.dropdownMenuEntries) {
      ...
      final bool isCurrentSelectionValid = filteredEntries.any(
              (MyDropdownMenuEntry<T> entry) => entry.label == _localTextEditingController?.text
      );
  ...
  List<MyDropdownMenuEntry<T>> filter(List<MyDropdownMenuEntry<T>> entries, TextEditingController textEditingController) {
    ...
    List<MyDropdownMenuEntry<T>> filteredEntries = entries
        .where((MyDropdownMenuEntry<T> entry) => entry.label.toLowerCase().contains(filterText))
        .toList();
    filteredEntries.sort((a, b) => a.label.toLowerCase().compareTo(b.label.toLowerCase()));
    return filteredEntries;
  }

  bool _shouldUpdateCurrentHighlight(List<MyDropdownMenuEntry<T>> entries) {
  ...
  int? search(List<MyDropdownMenuEntry<T>> entries, TextEditingController textEditingController) {
    ...
    final int index = entries.indexWhere((MyDropdownMenuEntry<T> entry) => entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(
      List<MyDropdownMenuEntry<T>> filteredEntries,
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      final MyDropdownMenuEntry<T> entry = filteredEntries[i];
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder: (BuildContext context, MenuController controller, Widget? child) {
        ...
        final Widget textField = TextField(
            ...
            onEditingComplete: () {
              if (currentHighlight != null) {
                final MyDropdownMenuEntry<T> entry = filteredEntries[currentHighlight!];
                if (entry.enabled) {
                  _localTextEditingController?.value = TextEditingValue(
                    text: entry.label,
                    selection: TextSelection.collapsed(offset: entry.label.length),
